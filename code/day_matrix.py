...
for file_name in target_list:
    ...
    data_use = data[['open', 'high', 'low', 'close', 'volume', 'amount', 'PE', 'PB', 'PS', 'MKV', 'BP', 'volatility',
                     'ER']]
    x_train = np.array(data_use['2011':'2019'][:-1])
    x_train[np.isnan(x_train)] = 0
    y_train = np.array(data_use['2011':'2019']['close'][1:])
    y_train[np.isnan(y_train)] = 0
    x_test = np.array(data_use['2020':][:-1])
    x_test[np.isnan(x_test)] = 0
    y_test = np.array(data_use['2020':]['close'][1:])
    ...
    # model.save('../model/day_matrix/'+file_name+'.h5')
    # print('模型保存完成')
    # # 绘制loss
    # plt.figure()
    ...
    # plt.legend()
    # plt.title('Loss')
    # plt.savefig('../figure/day_matrix/loss_'+file_name+'.png')
    ...
    # plt.ylabel('Price')
    # plt.legend()
    # plt.savefig('../figure/day_matrix/pred_'+file_name+'.png')
    # plt.clf()  # 清除当前图形及其所有轴，但保持窗口打开，以便可以将其重新用于其他绘图。
    # plt.close()  # 完全关闭图形窗口
