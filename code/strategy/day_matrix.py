...
def on_data(context):
    ...
    YZ = get_reg_factor(reg_idx=context.reg_factor[0], target_indices=(), length=1, df=True)
    PEs = YZ[YZ['factor'] == 'PE']
    ti = PEs['target_idx']
    date = PEs['date']
    factor = PEs['factor']
    ...
    diff_dic = {}
    model_list = os.listdir('../../model/day_matrix')
    code_list = []
    for i in model_list:
    ...
    for ind in range(len(model_list)):
        print(ind)
        PE = list(PEs.value)[ind]
        PB = list(PBs.value)[ind]
        # PCF = YZ[(YZ['factor'] == 'PCF') & (YZ['target_idx'] == ind)]['value']
        PS = list(PSs.value)[ind]
        MKV = list(MKVs.value)[ind]
        # ARBR = YZ[(YZ['factor'] == 'ARBR') & (YZ['target_idx'] == ind)]['value']
        BP = list(BPs.value)[ind]
        open = list(df_day.open)[ind]
        high = list(df_day.high)[ind]
        low = list(df_day.low)[ind]
        close = list(df_day.close)[ind]
        volume = list(df_day.volume)[ind]
        amount = list(df_day.amount)[ind]
        # gains = list(df_day.gains)[ind]
        volatility = list(df_day.volatility)[ind]
        ER = list(df_day.ER)[ind]
        data_use = [[open, high, low, close, volume, amount, PE, PB, PS, MKV, BP, volatility, ER]]
        data_use = np.array(data_use)
        # print(data_use.shape)
        data_use = data_use.reshape(data_use.shape[0], 1, data_use.shape[1])
        model = load_model('../../model/day_matrix/' + model_list[ind])
        ...
        # print(data_use)
        diff_dic[code_list[ind]] = (model.predict(data_use)[0][0] - close)
...
if __name__ == '__main__':
    ...
    for i in model_list:
        code_list.append(re.findall('(.{1,}).h5', i)[0])
    run_backtest(strategy_name='DL_day_matrix5', file_path='.', target_list=code_list,
