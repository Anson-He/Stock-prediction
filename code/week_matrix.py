...
week = os.listdir("../model/week_matrix")
...
for file_name in target_list:
    if file_name + '.h5' not in week:
        ...
        while end < s:
            ...
            y_train.append(np.array(data_use.iloc[end:end+7]['close']).tolist())
            end = end + 7
            start = end - 30
        x_test = []
        y_test = []
        ...
        while end < s:
            ...
            y_test.append(np.array(data_use.iloc[end:end+7]['close']).tolist())
            end = end + 7
            start = end - 30
        x_train = np.array(x_train)
        while len(y_train[-1]) != len(y_train[-2]):
            y_train[-1].append(sum(y_train[-1])/len(y_train))
        y_train = np.array(y_train)
        x_test = np.array(x_test)
        while len(y_test[-1]) != len(y_test[-2]):
            y_test[-1].append(sum(y_test[-1])/len(y_test))
        ...
        model = Sequential()
        # model.add(BatchNormalization(input_shape=(time_step, vector)))
        model.add(LSTM(units=64, input_shape=(time_step, vector),  return_sequences=True))
        model.add(Dense(32))
        model.add(LSTM(32, return_sequences=False))
        model.add(Dense(32))
        model.add(Activation("relu"))
        ...
        # model.save('../model/week_matrix/'+file_name+'.h5')
        # print('模型保存完成')
        # # 绘制loss
        # plt.figure()
        ...
        # plt.legend()
        # plt.title('Loss')
        # plt.savefig('../figure/week_matrix/loss_'+file_name+'.png')
        ...
        # plt.ylabel('Price')
        # plt.legend()
        # plt.savefig('../figure/week_matrix/pred_'+file_name+'.png')
        # plt.clf()  # 清除当前图形及其所有轴，但保持窗口打开，以便可以将其重新用于其他绘图。
        # plt.close()  # 完全关闭图形窗口
