...
month = os.listdir("../model/month_matrix")
...
for file_name in target_list:
    ...
    while end < s:
        ...
        y_train.append(np.array(data_use.iloc[end:end+30]['close']).tolist())
        end = end + 30
        start = end - 100
    x_test = []
    y_test = []
    ...
    while end < s:
        ...
        y_test.append(np.array(data_use.iloc[end:end+30]['close']).tolist())
        end = end + 30
        start = end - 100
    ...
    # model.save('../model/month_matrix/'+file_name+'.h5')
    # print('模型保存完成')
    # # 绘制loss
    # plt.figure()
    ...
    # plt.legend()
    # plt.title('Loss')
    # plt.savefig('../figure/month_matrix/loss_'+file_name+'.png')
    ...
    # plt.ylabel('Price')
    # plt.legend()
    # plt.savefig('../figure/month_matrix/pred_'+file_name+'.png')
    # plt.clf()  # 清除当前图形及其所有轴，但保持窗口打开，以便可以将其重新用于其他绘图。
    # plt.close()  # 完全关闭图形窗口
